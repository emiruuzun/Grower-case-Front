  ...
  const ComparisonCard = ({ title, data }) => {
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <h3 className="font-semibold text-lg text-gray-800 pb-3 mb-4 border-b border-gray-100">{title}</h3>
        <div className="space-y-6">
          <div>
            <div className="mb-4">
              <h4 className="text-sm font-medium text-gray-600 mb-3">Organik Trafik</h4>
              <div className="flex items-baseline">
                <span className="text-2xl font-bold text-gray-900">
                  {data.organicTraffic.currentValue}
                </span>
                <div className={`ml-3 px-2 py-0.5 rounded-full text-sm ${getTrendColor(data.organicTraffic.differencePercentage)} bg-opacity-10 ${getTrendColor(data.organicTraffic.differencePercentage).replace('text-', 'bg-')}`}>
                  {getTrendIcon(data.organicTraffic.differencePercentage)}
                  {Math.abs(data.organicTraffic.differencePercentage).toFixed(1)}%
                </div>
              </div>
            </div>

            <div>
              <h4 className="text-sm font-medium text-gray-600 mb-3">Website Trafiği</h4>
              <div className="flex items-baseline">
                <span className="text-2xl font-bold text-gray-900">
                  {data.websiteTraffic.currentValue}
                </span>
                <div className={`ml-3 px-2 py-0.5 rounded-full text-sm ${getTrendColor(data.websiteTraffic.differencePercentage)} bg-opacity-10 ${getTrendColor(data.websiteTraffic.differencePercentage).replace('text-', 'bg-')}`}>
                  {getTrendIcon(data.websiteTraffic.differencePercentage)}
                  {Math.abs(data.websiteTraffic.differencePercentage).toFixed(1)}%
                </div>
              </div>
            </div>
          </div>
